"""
Local Disk Backend — pure filesystem access without database.

Provides read/write/list/delete operations directly on the host filesystem.
No versioning, no trash, no database dependency.

Security:
- All paths are validated to stay within the configured host directory
- Binary files are detected and blocked from read operations
...
class LocalDiskBackend:
    ...
    def _resolve_path(self, virtual_path: str, follow_symlinks: bool = False) -> Path:
        ...
        Args:
            virtual_path: Path relative to mount root (e.g., "/src/main.py")
            follow_symlinks: If False (default), reject symlinks for security.
                This prevents attacks where a symlink could be swapped
                to point outside host_dir between validation and use.

        Returns:
            Resolved physical Path

        Raises:
            PermissionError: If the resolved path escapes host_dir or
                contains symlinks (when follow_symlinks=False)
        """
        ...
        # Security: Check each path component for symlinks (reject if found)
        # This prevents TOCTOU attacks where symlinks could be swapped
        ...
        # Now resolve to catch .. traversal
        resolved = candidate.resolve()

        # Security: ensure resolved path is within host_dir
        try:
    ...
    def _to_virtual_path(self, physical_path: Path) -> str:
        ...
        Raises ValueError if the path is outside host_dir (e.g., symlink target).
        """
    ...
    # =========================================================================
    # Raw Content Access (used by UnifiedFileSystem for cross-mount ops)
    # =========================================================================
    ...
    async def list_dir(self, path: str = "/") -> ListResult:
        ...
        def _scan():
            ...
            for entry in os.scandir(resolved):
                try:
                    # Skip symlinks that resolve outside host_dir
                    ...
                    continue  # Skip entries we can't stat or symlinks outside host
    ...
    async def write(
        ...
        def _write():
            """Atomic write; returns True if the file was newly created."""
        ...
        return WriteResult(
            ...
            file_path=path,
            created=created,
            version=1,  # Local files don't have versioning
        )
    ...
    async def edit(
        ...
        """Edit a file using smart text replacement (read-modify-write)."""
        valid, error = validate_path(path)
        if not valid:
    ...
    async def delete(self, path: str, permanent: bool = False) -> DeleteResult:
        ...
        Local mounts have no trash — all deletes are permanent and
        cannot be undone. The permanent parameter is accepted for
        interface compatibility but is always treated as True.
        """
        try:
            resolved = self._resolve_path(path)
        except PermissionError as e:
        ...
        return DeleteResult(
            ...
            file_path=path,
            permanent=True,  # Always permanent for local disk
        )
    ...
    async def mkdir(self, path: str, parents: bool = True) -> MkdirResult:
        ...
        def _mkdir():
            ...
            for p in reversed(parents_to_check):
                if p.exists():
                    created.append(self._to_virtual_path(p))
            return created
