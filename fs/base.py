"""
Base File System implementation with shared logic.

This module provides the common functionality for both LocalFileSystem
and DatabaseFileSystem. Subclasses implement storage-specific methods.
"""
...
class BaseFileSystem(ABC):
    ...
    async def _read_content(self, path: str) -> str | None:
        """Read file content from storage. Returns None if not found."""
        ...
    ...
    async def _commit(self, session: "AsyncSession") -> None:
        """Commit changes (flush for managed session, commit for owned)."""
        ...
    ...
    async def _get_file(
        ...
        if not include_deleted:
            query = query.where(WorkspaceFile.deleted_at.is_(None))
    ...
    async def _save_version(
        ...
        """Save current content as a version (called BEFORE overwriting)."""
        version = FileVersion(
    ...
    async def _ensure_parent_dirs(self, session: "AsyncSession", path: str) -> None:
        ...
        Uses MERGE to handle race conditions where another request might
        create the same directory concurrently.
        """
        schema = SQLModel.metadata.schema or "dbo"
        merge_sql = text(f"""
            MERGE INTO [{schema}].[workspace_files] WITH (HOLDLOCK) AS target
            USING (SELECT :user_id AS user_id, :path AS path) AS source
            ON target.user_id = source.user_id AND target.path = source.path
            WHEN NOT MATCHED THEN
                INSERT (file_id, user_id, path, name, is_directory, version, created_at, updated_at)
                VALUES (:file_id, :user_id, :path, :name, 1, 1, SYSDATETIMEOFFSET(), SYSDATETIMEOFFSET());
        """)
        ...
        try:
            ...
            if existing:
                ...
                if not existing.deleted_at:
                    ...
                    if old_content:
                        await self._save_version(session, existing, old_content, created_by)
            ...
            pass  # Session managed by subclass
    ...
    async def edit(
        ...
        try:
            ...
            if not result.success:
                return EditResult(success=False, message=result.error or "Edit failed", file_path=path)

            new_content = result.content
            assert new_content is not None

            # Save version before edit
            await self._save_version(session, file, content, created_by)
            ...
            return EditResult(
                success=True,
                message=f"Edit applied to {path} (v{file.version})",
                file_path=path,
                version=file.version
            )
        finally:
    ...
    async def delete(self, path: str, permanent: bool = False) -> DeleteResult:
        ...
        try:
            ...
            if permanent:
                # Hard delete children if directory
                if file.is_directory:
                    result = await session.execute(
                        select(WorkspaceFile).where(
                            WorkspaceFile.user_id == self.user_id,
                            WorkspaceFile.path.startswith(path + "/"),  # type: ignore[union-attr]
                        )
                    )
                ...
                file.path = to_trash_path(file.path, file.file_id)
    ...
    async def mkdir(self, path: str, parents: bool = True) -> MkdirResult:
        ...
        Uses MERGE to handle race conditions where multiple requests try
        ...
        try:
            ...
            while current != "/":
                ...
                if existing:
                    if not existing.is_directory:
                        return MkdirResult(success=False, message=f"Path exists as file: {current}")
                    break
            ...
            # Use MERGE to handle race conditions
            schema = SQLModel.metadata.schema or "dbo"
            merge_sql = text(f"""
                MERGE INTO [{schema}].[workspace_files] WITH (HOLDLOCK) AS target
                USING (SELECT :user_id AS user_id, :path AS path) AS source
            ...
            for dir_path in dirs_to_create:
                ...
                result = await session.execute(merge_sql, {
                    "file_id": str(uuid.uuid4()),
                    "user_id": self.user_id,
                    "path": dir_path,
                    "name": name,
                })
                # rowcount > 0 means the insert happened (not a conflict)
            ...
            if created_dirs:
                ...
                # All directories already existed (created by concurrent request)
                return MkdirResult(
                    success=True,
                    message=f"Directory already exists: {path}",
                    path=path,
    ...
    async def list_dir(self, path: str = "/") -> ListResult:
        ...
        try:
            ...
            # Get all files and filter to immediate children
            result = await session.execute(
                select(WorkspaceFile).where(
                    WorkspaceFile.user_id == self.user_id,
            ...
            for f in all_files:
                ...
                if path == "/":
                    # Top level - no slashes after leading /
                    if f.path.startswith("/") and "/" not in f.path[1:] and f.name not in seen:
                        seen.add(f.name)
    ...
    def _file_to_info(self, f: WorkspaceFile) -> FileInfo:
        """Convert WorkspaceFile to FileInfo."""
        return FileInfo(
            path=f.path,
            name=f.name,
    ...
    async def move(self, src: str, dest: str) -> MoveResult:
        ...
        try:
            ...
            # Update descendants if directory
            if src_file.is_directory:
                result = await session.execute(
                    select(WorkspaceFile).where(
                        WorkspaceFile.user_id == self.user_id,
                        WorkspaceFile.path.startswith(src + "/"),  # type: ignore[union-attr]
                    )
                )
    ...
    async def list_versions(self, path: str) -> list[VersionInfo]:
        ...
        try:
            ...
            result = await session.execute(
                select(FileVersion)
                .where(FileVersion.file_id == file.file_id)
                .order_by(FileVersion.version.desc())  # type: ignore[union-attr]
            )
    ...
    async def restore_version(self, path: str, version: int) -> RestoreResult:
        ...
        try:
            file = await self._get_file(session, path)
            if not file:
                return RestoreResult(success=False, message=f"File not found: {path}")

            result = await session.execute(
                select(FileVersion)
                .where(FileVersion.file_id == file.file_id)
                .where(FileVersion.version == version)
            )
            ...
            old_content = version_record.content
        finally:
            await self._commit(session)

        # Write restored content (saves current as version first)
        write_result = await self.write(path, old_content, created_by="restore")
    ...
    async def get_version_content(self, path: str, version: int) -> str | None:
        ...
        try:
            ...
            version_record = result.scalar_one_or_none()
            return version_record.content if version_record else None
        finally:
            await self._commit(session)
    ...
    async def list_trash(self) -> ListResult:
        ...
        try:
            result = await session.execute(
                select(WorkspaceFile).where(
                    WorkspaceFile.user_id == self.user_id,
                    WorkspaceFile.deleted_at.is_not(None),  # type: ignore[union-attr]
                )
            )
    ...
    async def restore_from_trash(self, path: str) -> RestoreResult:
        ...
        try:
            result = await session.execute(
                select(WorkspaceFile).where(
                    WorkspaceFile.user_id == self.user_id,
                    WorkspaceFile.original_path == path,
                    WorkspaceFile.deleted_at.is_not(None),  # type: ignore[union-attr]
                )
            )
