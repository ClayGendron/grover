...
3. **Semantic search** — HNSW vector index (usearch) with pluggable embedding providers.
...
```
src/grover/
├── __init__.py             # Public exports: Grover, GroverAsync, Ref, SearchResult, file_ref
...
├── fs/                     # Filesystem layer
│   ├── protocol.py         # StorageBackend + capability protocols (SupportsVersions, etc.)
...
├── graph/
│   ├── _graph.py           # Graph — rustworkx wrapper (add/remove nodes/edges, queries)
...
├── search/
│   ├── _index.py           # SearchIndex — usearch HNSW wrapper
│   ├── extractors.py       # Text extraction (chunks → EmbeddableChunk)
│   └── providers/
│       ├── _protocol.py    # EmbeddingProvider protocol
│       └── sentence_transformers.py  # Default provider (all-MiniLM-L6-v2)
...
├── test_graph.py           # Graph node/edge ops, queries, traversal
├── test_analyzers.py       # Python/JS/TS/Go code analysis + chunk extraction
├── test_search.py          # Vector search indexing, similarity, persistence
...
1. **Everything is a file** — no non-file entities, no URIs. Graph nodes = file paths. Chunks = files in `.grover/chunks/`.
...
All three checks (lint, type check, tests) must pass before committing.
...
5. **Run all quality checks.** All three must pass:
   ```
